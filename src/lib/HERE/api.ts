...
export default async function getRoute(
  options: RouteInputTyp
): Promise<getRouteReturnTyp | null> {
  try {
    // geojson parser
    const $ = new GeoJSON();
    ...
    const srcGeoCode = (
      await axios.get(
        `${HEREconfig.geocodeEndpoint}?q=${options.origin}&apiKey=${HEREconfig.hereApiKey}`
      )
    ...
    const destGeoCode = (
      await axios.get(
        `${HEREconfig.geocodeEndpoint}?q=${options.destination}&apiKey=${HEREconfig.hereApiKey}`
      )
    ).data.items[0].position;

    let result = (
      await axios.get(
        `${HEREconfig.routesEndpoint}?transportMode=${
          options.transportMode
        }&origin=${(srcGeoCode.lat, srcGeoCode.lng)}&destination=${
          (destGeoCode.lat, destGeoCode.lng)
        }&return=summary,polyline&apiKey=${HEREconfig.hereApiKey}`
      )
